...
for r in rows:
    fail=r['requires_mfa'] and not r['mfa']
    decision='WRONG_AUDIENCE' if not r['audience_ok'] else ('WOULD_BLOCK' if fail and r['mode']=='report-only' else ('BLOCK' if fail else 'ALLOW'))
